...
const insightHtml = (paragraphs: string[]) =>
  `<section class="chart-insights"><h3>このグラフからわかること</h3>${paragraphs.map((p) => `<p>${escape(p)}</p>`).join("")}</section>`;
export function bars(
  ...
  return `<figure class="data-chart"><figcaption>${escape(title)}</figcaption>${insightHtml(comparisonInsights(rows, metric, signed))}<p class="note">${signed ? "単位：人。縦線は0人。左は減少、右は増加。同じ尺度で比較しています。" : "単位：人。0を起点に、棒の長さで人数を比較しています。"}</p><div class="bar-axis"><span>${number(min)}</span><span>${number(max)} 人</span></div>${rows.map((r) => `<div class="bar-row"><span>${escape(r.label)}</span><span class="bar-track" aria-hidden="true"><i class="bar-zero" style="left:${zero}%"></i><i class="bar-fill" style="left:${((Math.min(0, r.value) - min) / span) * 100}%;width:${(Math.abs(r.value) / span) * 100}%"></i></span><strong>${signed && r.value > 0 ? "+" : ""}${number(r.value)}</strong></div>`).join("")}</figure>`;
}
