...
2026-09-06: 市民向けの独自デザインシステムと既存テーマを統合する[リデザイン計画](REDESIGN-PLAN.md)を作成しました。
現状監査、情報設計、方向比較、試作、市民検証、全画面移行の手順を整理した計画案です。画面改修は未着手です。
...
文章検証・訂正の公開接続は完了し、人口動態と年齢構成の拡充へ進んでいます。対象・操作方法・残る条件は[FACT-CHECK.md](FACT-CHECK.md)、実測結果は[STATUS.md](STATUS.md)に記録します。
...
ユーザーの2026-09-06の指示により、許可された機能・表示変更は、必要な検証とレビュー後にGitHub・本番への反映と公開URLでの確認まで続けます。ローカル実装だけを完了とはしません。認証や新しい費用・対外連絡などの追加条件が必要な場合は、その具体的な条件を伝えます。

2026-09-06の就寝中の実行委任に基づく成果・検証・残る順序は[実装進捗](IMPLEMENTATION-PROGRESS.md)にまとめています。

## 2026-09-06: 本文を主役にする

ユーザーの追加指示により、数値の読み上げではなく、規模・トレンドの転換・暮らしへの意味・議論の問いまで説明する段落を中心にします。本文→根拠のグラフ→必要なときに開く表の順です。

過去と直近、総数と世代・地区の違い、人口と世帯、移動・アクセス、費用と担い手を読み合わせることを編集基準にします。分野横断の不足や効果が未確認なら、結論の代わりに確かめる資料と判断の選択肢を具体化します。今回の委任範囲はこの説明と議論の提示であり、特定政策の決定や専門家監修の代行ではありません。
